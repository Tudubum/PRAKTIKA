...
//3) Naudojantis scripts/data.js viduje esančiais duomenimis:
  ...
  // innerHTML +=, nes pridedame elementus. Jeigu tik = tuomet jau replace'inam
    ...
    <div class="card_Container">
      ...
      <div class="textContainer">
        <div class="pav_metai"> 
          ...
          <h3>(${data.movies[i].leidimoMetai})</h3>
        </div>
        <h5>Trukmė: ${data.movies[i].trukme} min.</h5>
        <h5>Tipas: ${data.movies[i].tipas}</h5>
        <h5>IMDB :${data.movies[i].IMDB}</h5>
        <h4>Aprašymas:</h4>
        <p>${data.movies[i].aprasymas}</p>
        <h5>Režisierius: ${data.movies[i].rezisierius}</h5>
        <h5>Aktoriai: ${data.movies[i].aktoriai}</h5>
      </div>
    </div>
    `
  }
 ...
 //3.2) Sukurkite mygtuką, kurį paspaudus būtų sugeneruojami visi filmai.
  ...
  //console.log(aktoriai, IMDB, leidimoMetai, pavadinimas, rezisierius, tipas, trukme, paveiksliukas, aprasymas);

let btn = document.querySelector('button');
let korteles = document.querySelector('cards');
...
/*let paieska = document.querySelector('[data]');
data.forEach(user => {
  const card = data.content.cloneNode(true).children[0]
  console.log(user)
})
  ...
  //console.log(e);
//let moviesList = document.querySelector(".cards");
//const [aktoriai, IMDB, leidimoMetai, pavadinimas, rezisierius, tipas, trukme, paveiksliukas, aprasymas] = [
  //e.target.elements.aktoriai.value,
  //e.target.elements.IMDB.value,
  //e.target.elements.leidimoMetai.value,
  //e.target.elements.pavadinimas.value,
  //e.target.elements.rezisierius.value,
